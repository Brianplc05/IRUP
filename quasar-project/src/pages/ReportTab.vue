<template>
  <div id="q-app" style="min-height: 100vh;">
    <div class="q-pa-sm row items-start q-gutter-xs">
      <div class="my-card">
        <div style="height: 385px; display: flex; justify-content: flex-start; margin-left: 25px;">
          <AIRadialBar :series="AIchartSeries" :labels="aichartLabels" />
          <RadialBar :series="MDchartSeries" :labels="chartLabels" style="margin-left: 10px;"/>
        </div>
...
export default defineComponent({
  ...
  computed: {
    ...
    MDchartSeries() {
      ...
      const matchCount = this.disdeptMatch.reduce((acc, item) => acc + item.MatchCount, 0);
      const noMatchCount = this.disdeptMatch.reduce((acc, item) => acc + item.NoMatchCount, 0);
      return [matchCount, noMatchCount];
    },

    AIchartSeries(){
      if (this.distotalAction.length === 0) return [0, 0];
      const matchCount = this.distotalAction.reduce((acc, item) => acc + item.TotalActionItems, 0);
      const noMatchCount = this.distotalAction.reduce((acc, item) => acc + item.ActionStatus2Count, 0);
      return [matchCount, noMatchCount];
    }
  },
  ...
  methods: {
    async getCountSubject() {
      try {
        ...
        console.log('may lamang:', this.getCountSubs)
    ...
    async getMatchDepartments() {
      try {
        ...
        console.log('may lamang dept:', this.getCountSubs)
        this.disdeptMatch = this.getCountSubs;
      } catch (error) {
        console.error("Error fetching data:", error);
      }
    },
    ...
    async getTotalActions() {
      try {
        ...
        console.log('may lamang action:', this.getCountSubs)
